...
export type ReaderProps = Partial<{
  ...
  onZoomChanged: (event: CustomEvent<{ zoom: number; level: number }>) => void;
  onThemeChanged: (
    event: CustomEvent<{ theme: number; level: number }>,
  ) => void;
  onError: (event: CustomEvent<{ message: string; type: string }>) => void;
}> &
  React.HTMLAttributes<HTMLElement>;

export declare const Reader: React.FC<ReaderProps>;

declare global {
  namespace JSX {
    interface IntrinsicElements {
      'readiant-reader': ReaderProps;
    }
  }
}
