...
export class Builder {
    ...
    static animateLeft() {
        ...
        if (this.pendingAnimation || this.isAnimating)
            return;
    ...
    static async setStylesheet(id, fonts, stylesheetText) {
        ...
        this.fontLoadPromise = this.waitForFontsToLoad();
    ...
    static textElement(textElement, content, index, viewBox, ignore) {
        ...
        for (let wi = 0; wi < nonEmptyWords.length; wi++) {
            ...
            const startXValue = textElement.x[map.startI];
            let left = 0;
            ...
            if (wordIdx === 0 && left > 0) {
                ...
                left -= offset;
            const endXIdx = nextMap !== undefined ? nextMap.startI : map.i + (hasSpacesInX ? 1 : 0);
    ...
    static textLayer(elements, textContent, side, viewBox) {
        ...
        const entries = [...textElementsAndContent.values()];
        ...
        for (let word of words) {
            ...
            if (sentence.trim() === '' && !isPurePunctuation) {
                ...
                if (rest.length > 0) {
                    ...
                    sentence =
                        restIdx > -1
                            ? sentence.slice(0, restIdx) +
                                sentence.slice(restIdx + rest.length)
                            : sentence;
                    rest = '';
                }
                sentenceWords = sentence
            ...
            const syntaxWordTrimmed = syntaxWord.trimStart();
            const syntaxWordIndex = syntaxWordTrimmed.startsWith(normalizedWord)
                ? syntaxWord.indexOf(normalizedWord)
                : -1;
            if (syntaxWordIndex > -1) {
                ...
                while (lookahead <= wordIndex + 10 && lookahead < wordSyntax.length) {
                    const candidate = this.normalize(wordSyntax[lookahead].value, false);
                    if (candidate.startsWith(normalizedWord)) {
                        wordIndex = lookahead;
                        syntaxWord = candidate.substring(normalizedWord.length);
                        lookaheadFound = true;
                        syntaxMissStreak = 0;
                        break;
                    }
            ...
            if (!isPurePunctuation) {
                ...
                if (normalizedWordIndex > -1) {
                    ...
                    for (const [leftoverSi, leftover] of sentenceLeftovers) {
                        ...
                        if (leftoverIdx > -1) {
                            ...
                            word.setAttribute('data-w', String(leftover.nextWordIndex));
                            ...
                            for (const split of leftoverWord.split(' ')) {
                                ...
                                if (splitIdx > -1)
                                    leftover.sentenceWords[0] =
                                        leftover.sentenceWords[0].substring(splitIdx + split.length);
                ...
                for (const split of normalizedWord.split(' ')) {
            ...
            if (typeof syntax !== 'undefined') {
                ...
                word.addEventListener('click', (event) => {
                    const s = word.getAttribute('data-s');
            ...
            word.addEventListener('mouseenter', () => {
                this.startHighlightingSentence(side, word.getAttribute('data-s'), word.getAttribute('data-w'));
            });
...
Builder.fontLoadPromise = null;
Builder.isFontsReady = false;
Builder.pendingAnimation = false;
