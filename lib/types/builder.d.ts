...
export class Builder {
    ...
    static processTextElementContent(textElement, textContent, textElementsAndContent) {
        ...
        if (normalizedContent.length > 0) {
            ...
            for (const [key, existing] of textElementsAndContent) {
                if (!this.normalize(existing.original).includes(normalizedContent))
                    continue;
    ...
    static textElement(textElement, content, index, viewBox, ignore) {
        ...
        const effectiveLength = spaceAtEnd && characters.length - 1 === contentLength
            ? contentLength
            : characters.length;
        const hasLigatures = textElement.displayLength !== contentLength;
        ...
        for (let ci = 0; ci < effectiveLength; ci++) {
            ...
            if (effectiveLength === contentLength) {
                mapped.push({
        ...
        for (let wi = 0; wi < nonEmptyWords.length; wi++) {
            ...
            if (typeof endXValue !== 'undefined')
                ...
                const remainingWidth = 100 - previousWidth - offset;
        ...
        style="${this.direction === Direction.Rtl ? 'right' : 'left'}:${left === 0 || nonEmptyWords.length === 1 ? '0' : left.toFixed(2) + '%'};width:${spanWidth === 100 || nonEmptyWords.length === 1
                ? '100'
                : spanWidth.toFixed(2)}%;"
        tabindex="0"
        ></span>`);
            wordIdx++;
            previousWidth += spanWidth;
        }
        ...
        for (let ei = 0; ei < entries.length; ei++) {
            ...
            for (const textElement of textElementAndContent.elements) {
                ...
                if (div.children.length > 0) {
                    ...
                    const [, retryDiv] = this.textElement(textElement, textElement.content, textElementAndContent.index, viewBox, textElementAndContent.ignore);
