    ...
    static plainTextContent() {
        ...
        return sentenceElements.length > 0
            ? grouped
                ...
                .map((word) => `<span class="rdnt__plain-text-page__word" data-s="${this.escapeHTML(sentence[0].sentence)}" data-w="${this.escapeHTML(word.word)}">${this.escapeHTML(word.content)}</span>`)
                .join('')}</span>`)
                .join('')
                .replace(/\s+/g, ' ')
                .trim()
