...
export class Readiant {
    ...
    static errorHandler(message) {
        const root = ReadiantElement.instance?.shadowRoot ?? document.body;
        if (ReadiantElement.instance?.shadowRoot) {
            ReadiantElement.instance.shadowRoot.innerHTML = '';
        }
    ...
    parse() {
        ...
        if (ReadiantElement.instance) {
            // First check for document-id attribute and map it to id
