...
export class Readiant {
    ...
    static errorHandler(message) {
        ...
        document.body.removeAttribute('data-verify');
    ...
    constructor() {
        ...
        if (VARIANT === 'OFFLINE') {
            console.log(`Readiant Offline — v${VERSION}`);
    ...
    async connect() {
        ...
        if (VARIANT === 'OFFLINE')
            ...
            Stream.setMessageHandler(string.Connected, () => {
                Stream.backoff = [1000, 2500, 5000, 10000];
                if (!this.connected) {
                    Stream.setMessageHandler(string.PDFDocumentInfo, async (data) => this.connectPDF(data));
                    Stream.setMessageHandler(string.EPUBDocumentInfo, async (data) => this.connectEPUB(data));
                }
                Stream.send({
                    connection: connectionInfo().connection,
                    id: this.options.id,
                    page: this.options.page,
                    type: string.ClientInfo,
                    webP: supportsWebP,
                });
            });
            await Stream.setup();
    ...
    async populate() {
        ...
        const response = await fetch(`./docs/${this.options.id}/index.json`, {
            mode: 'same-origin',
        });
        ...
        for (const file of index.files) {
            ...
            if (file.startsWith(`${this.options.id}/audio/`)) {
            ...
            if (file.startsWith(`${this.options.id}/elements/`)) {
                const response = await fetch(`./docs/${file}.json`, {
                    mode: 'same-origin',
                });
                const elements = await this.replaceImageURLs((await response.json()), index.imageInfo);
                for (const [id, element] of elements)
                    Storage.storeElement(id, element);
                continue;
            }
            if (file.startsWith(`${this.options.id}/textContent/`)) {
                const response = await fetch(`./docs/${file}.json`, {
                    mode: 'same-origin',
                });
            ...
            if (file.startsWith(`${this.options.id}/definitions`)) {
                const response = await fetch(`./docs/${file}.json`, {
                    mode: 'same-origin',
                });
                const elements = (await response.json());
                await Builder.definitions(await this.replaceImageURLs(elements, index.imageInfo));
                continue;
            }
        }
        if (index.type === string.ePub)
    ...
    async replaceImageURL(definition, imageInfo) {
        ...
        for (const info of imageInfo)
            def = def.replaceAll(info.id, `./docs/${this.options.id}/images/${info.id}_4.${info.transparent ? 'png' : 'jpg'}`);
        return def;
    }
...
Readiant.menuButtons = document.getElementsByClassName('rdnt__menu__buttons')[0];
Readiant.pageNumber = document.getElementsByClassName('rdnt__page-number')[0];
