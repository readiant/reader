...
import { connectionInfo, hoverEvents, orientation, webP } from './detection.js';
...
export class Readiant {
    ...
    static async withContext(promise) {
    ...
    static get locale() {
        return this.localeMap.get(Readiant.root);
    }
    static set locale(val) {
        if (typeof val === 'undefined')
            this.localeMap.delete(Readiant.root);
        else
            this.localeMap.set(Readiant.root, val);
    }
    static get preview() {
        return this.previewMap.get(Readiant.root) ?? false;
    }
    static set preview(val) {
        this.previewMap.set(Readiant.root, val);
    }
    static get type() {
        return this.typeMap.get(Readiant.root) ?? ContentType.SVG;
    }
    static set type(val) {
        this.typeMap.set(Readiant.root, val);
    }
    ...
    static errorHandler(message) {
        ...
        const element = inst?.rootContext instanceof ShadowRoot
            ? inst.rootContext.host
            : undefined;
        if (element) {
    ...
    activateContext() {
        Readiant.root = this.rootContext;
    }
    abort() {
    ...
    constructor(root) {
        ...
        this.navigationState = {
            ...
            animationPages: [{ page: 2, position: PagePosition.Right }],
            chapterIndex: 0,
            currentPage: 1,
            currentPages: [{ page: 1, position: PagePosition.Right }],
            direction: Direction.Ltr,
            hasRegistered: false,
        };
        this.builderState = {
            activeFont: 'rdnt__font--original',
            activeFontSize: 'rdnt__font-size--8',
            activeLetterSpacing: 'rdnt__letter-spacing--3',
            activeLineHeight: 'rdnt__html-page--line-height--2',
            activeWordSpacing: 'rdnt__word-spacing--3',
            answerSentencesOnPage: new Map(),
            animationDisabled: false,
            animationEpoch: 0,
            animationCleanup: undefined,
            cachedElements: new Set(),
            cachedLinks: new Map(),
            currentImageQuality: undefined,
            currentSentenceIndex: 0,
            currentSide: PagePosition.Left,
            currentWordIndex: 0,
            direction: Direction.Ltr,
            expandingSpread: false,
            expansionTranslation: 0,
            expansionTransitionPending: false,
            forcingPortrait: false,
            ...
            layout: undefined,
            htmlOffset: 0,
            onPlainTextRendered: null,
            plainTextLinesObserver: null,
            previouslyShownPages: 0,
            styles: undefined,
            wantedElements: new Set(),
            elementsOnPage: new Map(),
            handlers: new Set(),
        };
        this.storageState = {
            cache: new Map(),
        ...
        this.audioState = {
            providers: new Set(),
            lineHighlighter: 1,
            availableAudio: undefined,
            element: undefined,
            endSentenceTime: undefined,
            endWordTime: undefined,
            playbackRate: 1,
            playingPage: 0,
            provider: undefined,
            pendingAudioTimeout: undefined,
            runningTimer: false,
            startTime: undefined,
            startSentenceTime: undefined,
            startWordTime: undefined,
            subtitles: 1,
            audioTimeout: 500,
            playback: new Map(),
            playingState: AudioPlayingState.Initial,
            timerType: 1,
        };
        this.zoomState = {
            level: 2,
        ...
        this.screenModeState = {
            screenMode: 1,
        };
        this.textModeState = {
            level: 1,
        };
        this.colorblindState = {
            active: undefined,
        };
        this.fontsState = {
            active: 'rdnt__font--original',
        };
        this.accepted = {
        ...
        this.fonts = {};
        if (isOffline) {
            this.fontAssetsPromise ?? (this.fontAssetsPromise = import('./fontAssets.js').then(({ fontAssets, }) => {
                this.fontAssets = JSON.parse(JSON.stringify(fontAssets));
            }));
        }
        else
            this.fontAssetsPromise ?? (this.fontAssetsPromise = Promise.resolve());
        if (root) {
            this.rootContext = root;
            Readiant.root = root;
        }
        else {
            this.rootContext = Readiant.root;
        }
        Readiant.instances.set(this.rootContext, this);
    ...
    async connect() {
        ...
        if (isOffline)
            ...
            const supportsWebP = await Readiant.withContext(webP());
            ...
            await Readiant.withContext(Stream.setup());
        }
    }
    async populate() {
        this.activateContext();
        ...
        const processFile = async (file) => {
            ...
            if (filePath.startsWith('audio/') || filePath.includes('/audio/')) {
                ...
                if (fileExtension === 'mp3') {
                    ...
                    Readiant.root = this.rootContext;
                    Storage.storeAudio(`${provider}__${fileNameWithoutExtension}`, audioData);
                }
            ...
            if (filePath.startsWith('fonts/') || filePath.includes('/fonts/')) {
                ...
                if (typeof this.fonts[fontName] === 'undefined')
                    this.fonts[fontName] = {};
                const fontMimeType = fileExtension === 'woff2' ? 'font/woff2' : 'font/woff';
                const fontBlob = new Blob([await response.arrayBuffer()], {
                    type: fontMimeType,
                });
                const fontUrl = await new Promise((resolve, reject) => {
                    const reader = new FileReader();
                    reader.addEventListener('load', () => {
                        if (typeof reader.result === 'string')
                            resolve(reader.result);
                        else
                            reject(new Error('Could not encode font.'));
                    });
                    reader.addEventListener('error', () => {
                        reject(reader.error ?? new Error('Could not encode font.'));
                    });
    ...
    async connectEPUB({ chapters, direction, document, indexes, pageCounts, translations, }) {
        ...
        this.activateContext();
        this.register({}, chapters, translations);
        this.activateContext();
    }
    async connectPDF({ availableAudio, chapters, document, inverted, offset, pages, rtl, spread, translations, }) {
        ...
        this.activateContext();
        this.register(availableAudio, chapters, translations);
        this.activateContext();
    }
    register(availableAudio, documentChapters, translations) {
        this.activateContext();
        if (this.abortController.signal.aborted)
            return;
    ...
    async parentMessageHandler(event) {
        ...
        switch (data.type) {
            ...
            case LogType.InitialPage:
                await Navigation.gotoPageDirectly(data.pages[0]);
                break;
    ...
    parse() {
        const options = {};
        const element = this.rootContext instanceof ShadowRoot
            ? this.rootContext.host
            : undefined;
        if (element) {
            const documentIdValue = element.getAttribute('document-id');
...
Readiant.localeMap = new Map();
Readiant.previewMap = new Map();
Readiant.typeMap = new Map();
