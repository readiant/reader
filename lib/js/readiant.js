...
export class Readiant {
    ...
    constructor(root) {
        ...
        if (isOffline) {
            this.fontAssetsPromise = import('./fontAssets.js').then(({ fontAssets, }) => {
                this.fonts = fontAssets;
            });
        }
        else {
            this.fontAssetsPromise = Promise.resolve();
            this.fonts = {};
        }
        if (root) {
            Readiant.root = root;
