...
export class Readiant {
    ...
    static errorHandler(message) {
        ...
        // Dispatch error event through Web Component
    ...
    async populate() {
        ...
        if (typeof this.options.url === 'undefined') {
            Readiant.errorHandler(new Error('Missing URL parameter.'));
            return;
        }
        ...
        for (const file of index.files) {
            ...
            if (file.includes(`${this.options.id}/audio/`)) {
            ...
            if (file.includes(`${this.options.id}/definitions`)) {
    ...
    async replaceImageURL(definition, imageInfo) {
        ...
        for (const info of imageInfo) {
            // Check if image has a signed URL, otherwise construct the path
            let imageUrl;
