...
export class Readiant {
    ...
    async populate() {
        ...
        const response = await fetch(`${this.options.url}index.json`, {
            mode: 'cors',
            credentials: 'omit',
            headers: {
        ...
        for (const file of index.files) {
            ...
            const response = await fetch(`${this.options.url}${file}`, {
                mode: 'cors',
                credentials: 'omit',
                headers: {
                    Accept: 'application/json, audio/mpeg, image/*, */*',
    ...
    parse() {
        const options = {};
        // Read from web component attributes if available
        ...
        // Fallback to URL parameters if no web component
