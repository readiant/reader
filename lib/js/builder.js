...
import { SpeechMarkType } from '../../../backend/src/db/models/types/Audio.js';
import { EffectiveConnectionType } from '../../../backend/src/db/models/types/Document.js';
import { Layout } from '../../../backend/src/epub/Layout.js';
...
export class Builder {
    ...
    static register(id) {
        ...
        link.setAttribute('href', VARIANT === 'OFFLINE'
            ? `/docs/${id}/stylesheet.css`
            : `/files/stylesheet/${id}`);
    ...
    static animateLeft() {
        ...
        const toFirst = Math.min(...Navigation.pages) === Navigation.currentPage - 1 &&
            this.animationPages.length === 1;
        const isLast = Math.max(...Navigation.pages) === Navigation.currentPage;
        const toLast = Math.max(...Navigation.pages) === Navigation.currentPage + 1 &&
            this.animationPages.length === 1;
        const translateX = parseFloat(this.animationLeft.style.width) / 2;
        if (window.getComputedStyle(this.animationLeft).animationName !== 'none') {
    ...
    static animateRight() {
        ...
        if (window.getComputedStyle(this.animationRight).animationName !== 'none') {
    ...
    static computeOrientation() {
        ...
        return window.matchMedia(`(max-width: ${String(this.PORTRAIT_WIDTH)}px)`)
            .matches
    ...
    static setDirection(direction) {
        if (direction === Direction.Rtl) {
            ...
            this.htmlPage.classList.add('rdnt__html-page--rtl');
        }
        else {
            Bar.syntaxElement.classList.remove('rdnt__bottom-bar__syntax--rtl');
            this.htmlPage.classList.remove('rdnt__html-page--rtl');
        }
    ...
    static filterTextElements(elements) {
                    ...
                    ...textElements,
                    {
                        content: this.direction === Direction.Rtl
                            ? String(exist.getAttribute('data-content'))
                                ...
                                .join('')
                            : String(exist.getAttribute('data-content')),
                        style: exist.getBoundingClientRect(),
                        transform,
    ...
    static findElementForQuery(query, element = document.body) {
    ...
    static font(activeFont, newFont) {
        document.body.classList.remove(activeFont);
    ...
    static async html(section, hrefResolver) {
        ...
        // Replace faulty closing of breaks
        ...
        if (section.layout === Layout.PrePaginated) {
            ...
            const style = window.getComputedStyle(this.viewport, null);
            const scale = Readiant.preview && !Fullscreen.active
                ? (window.screen.width -
                    (parseFloat(style.paddingLeft) +
                        parseFloat(style.paddingRight))) /
                    section.viewport[0]
                : Math.min((window.screen.width -
                    (parseFloat(style.paddingLeft) +
                        parseFloat(style.paddingRight))) /
                    section.viewport[0], (window.screen.height -
    ...
    static get isLeftMostPage() {
        return specGetScrollLeft(this.htmlPage) === 0;
    }
    ...
    static layer(side) {
        if (this.isAnimating)
            return;
        const computed = this.getPage(side).getBoundingClientRect();
        const viewport = this.viewport.getBoundingClientRect();
        if (side === PagePosition.Left) {
            this.leftLayer.style.height = `${String(computed.height)}px`;
            this.leftLayer.style.left = `${String(computed.x + window.scrollX)}px`;
            this.leftLayer.style.top = `${String(computed.y + window.scrollY)}px`;
            this.leftLayer.style.width = `${String(computed.width)}px`;
            this.animationLeft.style.height = `${String(computed.height)}px`;
            this.animationLeft.style.left = `${String(computed.x + window.scrollX)}px`;
            this.animationLeft.style.top = `${String(computed.y + window.scrollY)}px`;
            ...
            this.rightLayer.style.left = `${String(computed.x + window.scrollX)}px`;
            this.rightLayer.style.top = `${String(computed.y + window.scrollY)}px`;
            this.rightLayer.style.width = `${String(computed.width)}px`;
            this.animationRight.style.height = `${String(computed.height)}px`;
            this.animationRight.style.left = `${String(computed.x + window.scrollX)}px`;
            this.animationRight.style.top = `${String(computed.y + window.scrollY)}px`;
    ...
    /*private static layerHtml(): void {
      ...
      this.htmlLayer.style.left = `${x + window.scrollX}px`;
      this.htmlLayer.style.top = `${y + window.scrollY}px`;
    ...
    static observe() {
        ...
        if (Readiant.type === ContentType.SVG &&
            window.screen.width <= this.PORTRAIT_WIDTH &&
            ...
            for (const element of elements) {
                if (element.tagName.toLowerCase() === 'use') {
                    ...
                    if (href !== null && this.cachedLinks.has(href.substring(1)))
                        window.open(String(this.cachedLinks.get(href.substring(1))), '_blank');
                }
            }
        }
    }
    ...
    static async plainTextLines() {
        await this.waitForNextFrame();
        const computedStyle = window.getComputedStyle(this.plainTextPageContent, null);
    ...
    static resize() {
        const elements = [
            ...document.querySelectorAll('.rdnt__menu__buttons-right > .hidden'),
        ];
    ...
    static resizeHTML() {
        const forcePortrait = window.screen.width <= this.PORTRAIT_WIDTH ||
    ...
    static resizeSVG() {
        ...
        if (window.screen.width <= this.PORTRAIT_WIDTH &&
            Orientation.mode === OrientationMode.Landscape) {
            Orientation.toggle();
            Orientation.disableOrientationChange = true;
        }
        else if (window.screen.width > this.PORTRAIT_WIDTH &&
    ...
    static screenMode(currentScreenMode, screenModeLevel) {
        document.body.classList.remove(`rdnt__mode-${String(currentScreenMode)}`);
        document.body.classList.add(`rdnt__mode-${String(screenModeLevel)}`);
    }
    ...
    static scrollToResult(query) {
        const element = this.findElementForQuery(query);
    ...
    static stopHighlighting(side) {
        const words = typeof side !== 'undefined' && TextMode.level === 1
            ...
            : document.querySelectorAll(`.${CLASS_HIGHLIGHT_ACTIVE}`);
        for (const word of words) {
    ...
    static stopHighlightingSyntax(side) {
        const words = typeof side !== 'undefined'
            ...
            : document.querySelectorAll(`.${CLASS_HIGHLIGHT_SYNTAX_ACTIVE}`);
        for (const word of words) {
    ...
    static textElement(textElement, content, index, viewBox, ignore) {
        ...
        div.style.height = `${((textElement.style.height / viewBox[3]) *
            100).toFixed(2)}%`;
        div.style.left = `${((Math.abs(textElement.style.x + window.scrollX - viewBox[0]) /
            viewBox[2]) *
            100).toFixed(2)}%`;
        div.style.top = `${(100 -
            (Math.abs(textElement.style.y + window.scrollY + viewBox[1]) /
                viewBox[3]) *
                100).toFixed(2)}%`;
    ...
    static textLayer(elements, textContent, side, viewBox) {
        if (textContent.content.length === 0)
            return;
        const fragment = document.createDocumentFragment();
        const textElements = this.filterTextElements(elements);
        const textElementsAndContent = new Map();
        for (const textElement of textElements) {
            let closest;
            const narrowed = textContent.content
                .map((textContent, index) => ({
                content: textContent.content,
                ignore: textContent.ignore,
                index,
                original: typeof textContent.originalText !== 'undefined'
                    ? textContent.originalText
                    : textContent.content,
                transform: textContent.transform,
            }))
                .filter((textContent) => Math.abs(textContent.transform[5] - textElement.transform[5]) <
                Math.ceil(textElement.style.height));
            if (narrowed.length === 1)
                closest = narrowed[0];
            else if (narrowed.length > 1) {
                const narrowedByContent = narrowed.filter((textContent) => textContent.original === textElement.content);
                if (narrowedByContent.length === 1)
                    closest = narrowedByContent[0];
                else {
                    const narrowedByContentStart = narrowed.filter((textContent) => textContent.original.startsWith(textElement.content));
                    if (narrowedByContentStart.length === 1)
                        closest = narrowedByContentStart[0];
                    else {
                        const narrowedByContentIncludes = (narrowedByContentStart.length > 1
                            ? narrowedByContentStart
                            : narrowed).filter((textContent) => textContent.original
                            .replace(/\s+/g, ' ')
                            .includes(textElement.content));
                        if (narrowedByContentIncludes.length === 1)
                            closest = narrowedByContentIncludes[0];
                    }
                }
            }
            if (typeof closest === 'undefined')
                closest = textContent.content
                    .map((textContent, index) => ({
                    content: textContent.content,
                    ignore: textContent.ignore,
                    index,
                    ...
                    .reduce((a, b) => this.distance([textElement.transform[4], textElement.transform[5]], [a.transform[4], a.transform[5]]) <
                    this.distance([textElement.transform[4], textElement.transform[5]], [b.transform[4], b.transform[5]])
                    ? a
                    : b);
            if (textElementsAndContent.has(closest.index)) {
                const textElementAndContent = textElementsAndContent.get(closest.index);
                textElementsAndContent.set(closest.index, {
                    ...textElementAndContent,
                    elements: [...textElementAndContent.elements, textElement],
                });
            }
            else
                textElementsAndContent.set(closest.index, {
    ...
    static async textPosition() {
        ...
        const page = !isLeftVisible || this.leftTextLayer.children.length === 0
            ? this.rightTextLayer
            : this.leftTextLayer;
        const offset = window.getComputedStyle(this.viewport);
    ...
    static textSentences(sentences, side, syntax) {
        ...
        let sentenceIndex = 0;
        let sentenceWordsIndex = 0;
        let sentence = this.normalize(sentences[sentenceIndex]).toLowerCase();
        let sentenceWords = sentence
            .split(' ')
            .filter((word) => word.trim() !== '')
            .map((word) => word.toLowerCase());
        ...
        for (let word of words) {
            if (typeof syntax !== 'undefined' && syntaxWord.trim() === '') {
                wordIndex++;
                syntaxWord = (typeof wordSyntax[wordIndex] !== 'undefined'
                    ? this.normalize(wordSyntax[wordIndex].value)
                    : '').toLowerCase();
            }
            if (sentence.trim() === '') {
                sentenceIndex++;
                sentenceWordsIndex++;
                sentence = this.normalize(sentences[sentenceIndex]).toLowerCase();
                if (rest.length > 0) {
                    sentence = sentence.replace(rest, '');
                    rest = '';
                }
            ...
            const dataSentence = String(sentenceIndex);
            const dataWord = String(sentenceWordsIndex);
            let normalizedWord = this.normalize(String(word.getAttribute('data-word'))).toLowerCase();
...
_a = Builder;
Builder.animationLeft = document.getElementsByClassName('rdnt__animation--left')[0];
Builder.animationLeftFront = document.getElementsByClassName('rdnt__animation--left-front')[0];
Builder.animationLeftBack = document.getElementsByClassName('rdnt__animation--left-back')[0];
Builder.animationRight = document.getElementsByClassName('rdnt__animation--right')[0];
Builder.animationRightFront = document.getElementsByClassName('rdnt__animation--right-front')[0];
Builder.animationRightBack = document.getElementsByClassName('rdnt__animation--right-back')[0];
Builder.defs = document.getElementsByClassName('rdnt__defs')[0];
Builder.elems = document.getElementsByClassName('rdnt__elements')[0];
Builder.htmlLayer = document.getElementsByClassName('rdnt__layer--html')[0];
Builder.htmlTextLayer = document.getElementsByClassName('rdnt__text--html')[0];
Builder.htmlPage = document.getElementsByClassName('rdnt__html-page')[0];
Builder.htmlPageContent = document.getElementsByClassName('rdnt__html-page--content')[0];
Builder.layers = document.getElementsByClassName('rdnt__layers')[0];
Builder.left = document.getElementsByClassName('rdnt__page--left')[0];
Builder.leftLayer = document.getElementsByClassName('rdnt__layer--left')[0];
Builder.leftTextLayer = document.getElementsByClassName('rdnt__text--left')[0];
Builder.plainTextPage = document.getElementsByClassName('rdnt__plain-text-page')[0];
Builder.plainTextPageContent = document.getElementsByClassName('rdnt__plain-text-page__content')[0];
Builder.plainTextPageLines = document.getElementsByClassName('rdnt__plain-text-page__lines')[0];
Builder.progress = document.getElementsByClassName('rdnt__progress-value')[0];
Builder.right = document.getElementsByClassName('rdnt__page--right')[0];
Builder.rightLayer = document.getElementsByClassName('rdnt__layer--right')[0];
Builder.rightTextLayer = document.getElementsByClassName('rdnt__text--right')[0];
Builder.wordLabel = String(document.getElementsByClassName('rdnt__i18n')[0].getAttribute('data-word-label'));
Builder.viewport = document.getElementsByClassName('rdnt__viewport')[0];
