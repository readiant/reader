    ...
    static textElement(textElement, content, index, viewBox, ignore) {
        ...
        for (let ci = 0; ci < effectiveLength; ci++) {
            ...
            if (effectiveLength === contentLength) {
                mapped.push({ content: content.substring(0, 1), i: ci + 1 });
                ...
                lastCharSkipped = false;
                mapped.push({ content: content.substring(0, charIdx + 1), i: ci + 1 });
        ...
        const wordMapped = mapped.reduce((acc, cur) => {
            ...
            for (const part of parts) {
                if (part.trim() === '')
                    acc.push({ content: '', i: cur.i, startI: cur.i });
                else {
        ...
        for (let wi = 0; wi < nonEmptyWords.length; wi++) {
            ...
            let left = (textElement.x[map.startI] / textElement.style.width) *
                100 *
            ...
            const spanWidth = typeof textElement.x[endXIdx] !== 'undefined'
                ? (textElement.x[endXIdx] / textElement.style.width) *
                    100 *
                    textElement.transform[0] -
                    previousWidth -
                    offset
