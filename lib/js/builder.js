...
export class Builder {
    ...
    static findAllMatchingContent(textElement, textContent) {
        ...
        const potentialMatches = textContent.content
            ...
            .filter((content) => Math.abs(content.transform[5] - textElement.transform[5]) <
            Math.ceil(textElement.style.height))
            .sort((a, b) => {
            const distanceA = this.distance([textElement.transform[4], textElement.transform[5]], [a.transform[4], a.transform[5]]);
            const distanceB = this.distance([textElement.transform[4], textElement.transform[5]], [b.transform[4], b.transform[5]]);
        ...
        if (section.layout === Layout.PrePaginated) {
            ...
            const scale = Readiant.preview && !Fullscreen.active
                ...
                : Math.min((window.screen.width -
                    (parseFloat(style.paddingLeft) +
                        parseFloat(style.paddingRight))) /
                    section.viewport[0], (window.innerHeight -
