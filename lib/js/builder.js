...
export class Builder {
    ...
    static setStylesheet(id, fonts, stylesheetText) {
        if (typeof stylesheetText !== 'undefined') {
            ...
            if (fontFaceRules.length > 0) {
                ...
                fontStylesheet.textContent = fontFaceRules.join('\n');
    ...
    static font(activeFont, newFont) {
        const body = Readiant.root instanceof ShadowRoot
            ? Readiant.root.host
            : document.body;
        body.classList.remove(activeFont);
    ...
    static openLink(event) {
        if (typeof event.pageX !== 'undefined' &&
            ...
            const elements = Readiant.root.elementsFromPoint(event.pageX, event.pageY);
            for (const element of elements) {
    ...
    static screenMode(currentScreenMode, screenModeLevel) {
        const body = Readiant.root instanceof ShadowRoot
            ? Readiant.root.host
            : document.body;
        body.classList.remove(`rdnt__mode-${String(currentScreenMode)}`);
        body.classList.add(`rdnt__mode-${String(screenModeLevel)}`);
    }
    ...
    static scrollToResult(query) {
        const body = Readiant.root instanceof ShadowRoot
            ? Readiant.root.host
            : document.body;
        const element = this.findElementForQuery(query, body);
