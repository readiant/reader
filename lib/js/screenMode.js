...
export class ScreenMode {
    ...
    static change(event) {
        ...
        if (typeof event === 'number') {
            ...
            if (event > 3)
                event = 3;
            const element = document.querySelector(`[data-screen-mode="${String(event)}"]`);
            if (element === null)
                return;
            element.click();
...
ScreenMode.buttons = document.getElementsByClassName('rdnt__screen-mode');
ScreenMode.current = document.getElementsByClassName('rdnt__current-selection--screen-mode')[0];
