...
export class TextMode {
    ...
    static change(event) {
        ...
        if (typeof event === 'number') {
            ...
            if (event > 3)
                event = 3;
            const element = document.querySelector(`[data-text-mode="${String(event)}"]`);
            if (element === null)
                return;
            element.click();
...
TextMode.buttons = document.getElementsByClassName('rdnt__text-mode');
TextMode.current = document.getElementsByClassName('rdnt__current-selection--text-mode')[0];
TextMode.title = document.getElementsByClassName('rdnt__block-title--text-mode')[0];
TextMode.level = 1;
