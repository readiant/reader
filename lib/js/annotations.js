...
export class Annotations {
    ...
    static get state() {
        ...
        return inst.annotationsState;
    }
    static get annotations() {
        return this.state?.annotations ?? {};
    }
    static set annotations(val) {
        if (this.state)
            this.state.annotations = val;
    }
    ...
    static get color() {
        return this.state?.color ?? 'rgba(204,213,99,.3)';
    }
    ...
    static get size() {
        return this.state?.size ?? 20;
    }
    static set size(val) {
        if (this.state)
            this.state.size = val;
    }
    static get markerEvent() {
        return this.state?.markerEvent ?? [];
    }
    static set markerEvent(val) {
        if (this.state)
            this.state.markerEvent = val;
    }
    static get annotationHistory() {
        return this.state?.annotationHistory ?? [];
    }
    static set annotationHistory(val) {
        if (this.state)
            this.state.annotationHistory = val;
    }
    static get editComments() {
        return this.state?.editComments ?? false;
    }
    static set editComments(val) {
        if (this.state)
            this.state.editComments = val;
    }
    static get editMarkings() {
        return this.state?.editMarkings ?? false;
    }
    static set editMarkings(val) {
        if (this.state)
            this.state.editMarkings = val;
    }
    static get isErasing() {
        return this.state?.isErasing ?? false;
    }
    static set isErasing(val) {
        if (this.state)
            this.state.isErasing = val;
    }
    static get showAnnotations() {
        return this.state?.showAnnotations ?? false;
    }
    static set showAnnotations(val) {
        if (this.state)
            this.state.showAnnotations = val;
    }
    ...
    static register() {
        ...
        this.markerButton?.addEventListener('click', (event) => {
            event.preventDefault();
            this.toggleMarker();
        });
