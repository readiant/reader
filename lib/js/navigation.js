...
export class Navigation {
    ...
    static registerHTML(page, pageCounts, indexes, direction) {
        ...
        Stream.setMessageHandler(string.EPUBChapter, (chapter) => this.onSection(chapter));
        this.previousAction = {
            type: NavigationActionType.FirstLoad,
            chapterIndex: this.chapterIndex,
        };
        Stream.send({
            type: string.ChapterRequest,
            chapterIndex: this.chapterIndex,
        });
    ...
    static registerSVG(page, pages, pageCounts, direction, offset, spread) {
        ...
        Stream.setMessageHandler(string.Blueprint, (data) => {
            this.onBlueprint(data);
        });
        Stream.setMessageHandler(string.Definitions, (data) => Builder.definitions(data.payload));
        Stream.setMessageHandler(string.Elements, (data) => {
            this.onElements(data);
        });
        Stream.setMessageHandler(string.TextContent, (data) => {
            this.onTextContent(data);
        });
        if (VARIANT !== 'OFFLINE')
            Stream.send({
                preferredPage: this.currentPage,
                type: string.DefinitionRequest,
            });
    ...
    static gotoSearchResult(chapterIndex, query) {
        Stream.send({ type: string.ChapterRequest, chapterIndex });
        this.previousAction = {
    ...
    static requestPages(pages) {
        Stream.send({
            type: string.BlueprintRequest,
            pages,
        });
        Stream.send({
            type: string.CacheRequest,
            pages,
        });
        pages = pages.filter((page) => !Storage.hasText(page));
        if (pages.length > 0)
            Stream.send({
                type: string.TextContentRequest,
                pages,
            });
    }
...
Navigation.nextButton = document.getElementsByClassName('rdnt__navigation--next')[0];
Navigation.pageNumber = document.getElementsByClassName('rdnt__page-number')[0];
Navigation.pageNumberInput = document.getElementsByClassName('rdnt__page-number__input')[0];
Navigation.pageNumberProgress = document.getElementsByClassName('rdnt__page-number__progress')[0];
Navigation.pageNumberProgressValue = document.getElementsByClassName('rdnt__page-number__progress-value')[0];
Navigation.pageNumberTotal = document.getElementsByClassName('rdnt__page-number__total')[0];
Navigation.previousButton = document.getElementsByClassName('rdnt__navigation--prev')[0];
