...
export class Navigation {
    ...
    static get pageNumberProgress() {
        return Readiant.root.querySelector('.rdnt__page-number__progress');
    }
            ...
            if (this.pageNumberCurrent !== null)
                this.pageNumberCurrent.innerHTML =
                    this.currentPages.length === 1
                        ...
                        : `${String(this.currentPages[0].page + this.pageOffset)} ${this.and} ${String(this.currentPages[1].page + this.pageOffset)}`;
            for (const pageNumberTotal of this.pageNumberTotal)
    ...
    static touchHandler(action, event) {
        const isAnnotationsActive = Readiant.root.querySelector('.rdnt__markings') !== null
            ? Readiant.root
