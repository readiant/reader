...
export class Audio {
    ...
    static async onCanPlayThrough(time) {
        ...
        Readiant.windowContext.setTimeout(async () => {
            ...
            if (this.timerType === 4 || this.timerType === 5)
                return;
            await element.play();
        }, 10);
    ...
    static async start(time, side) {
        ...
        if (!this.availableAudio[this.provider].includes(Navigation.currentPage)) {
            if ((this.timerType === 4 || this.timerType === 5) && time === 0) {
                this.startTimerBeforeAudio(() => {
                    this.startTimerAfterAudio();
                });
            }
            else {
    ...
    static startTimerAfterAudio(seconds = 3) {
        ...
        if (this.timerType === 2 || this.timerType === 4) {
            Navigation.nextPage();
            return;
        }
        if (this.timerType === 3)
            return;
        if (this.timerType === 1 || this.timerType === 5) {
            this.runningTimer = true;
            Readiant.windowContext.setTimeout(() => {
                if (!this.runningTimer)
                    return;
