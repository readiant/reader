...
export class Audio {
    ...
    static get state() {
        ...
        return inst.audioState;
    }
    static get providers() {
        return this.state?.providers ?? new Set();
    }
    static get lineHighlighter() {
        return this.state?.lineHighlighter ?? 1;
    }
    static set lineHighlighter(val) {
        if (this.state)
            this.state.lineHighlighter = val;
    }
    static get availableAudio() {
        return this.state?.availableAudio;
    }
    static set availableAudio(val) {
        if (this.state)
            this.state.availableAudio = val;
    }
    static get element() {
        return this.state?.element;
    }
    static set element(val) {
        if (this.state)
            this.state.element = val;
    }
    static get endSentenceTime() {
        return this.state?.endSentenceTime;
    }
    static set endSentenceTime(val) {
        if (this.state)
            this.state.endSentenceTime = val;
    }
    static get endWordTime() {
        return this.state?.endWordTime;
    }
    static set endWordTime(val) {
        if (this.state)
            this.state.endWordTime = val;
    }
    static get playbackRate() {
        return this.state?.playbackRate ?? 1;
    }
    static set playbackRate(val) {
        if (this.state)
            this.state.playbackRate = val;
    }
    static get playingPage() {
        return this.state?.playingPage ?? 0;
    }
    static set playingPage(val) {
        if (this.state)
            this.state.playingPage = val;
    }
    static get provider() {
        return this.state?.provider;
    }
    static set provider(val) {
        if (this.state)
            this.state.provider = val;
    }
    static get hasProvider() {
        return this.provider !== undefined;
    }
    static get pendingAudioTimeout() {
        return this.state?.pendingAudioTimeout;
    }
    static set pendingAudioTimeout(val) {
        if (this.state)
            this.state.pendingAudioTimeout = val;
    }
    static get runningTimer() {
        return this.state?.runningTimer ?? false;
    }
    static set runningTimer(val) {
        if (this.state)
            this.state.runningTimer = val;
    }
    static get startTime() {
        return this.state?.startTime;
    }
    static set startTime(val) {
        if (this.state)
            this.state.startTime = val;
    }
    static get startSentenceTime() {
        return this.state?.startSentenceTime;
    }
    static set startSentenceTime(val) {
        if (this.state)
            this.state.startSentenceTime = val;
    }
    static get startWordTime() {
        return this.state?.startWordTime;
    }
    static set startWordTime(val) {
        if (this.state)
            this.state.startWordTime = val;
    }
    static get subtitles() {
        return this.state?.subtitles ?? 1;
    }
    static set subtitles(val) {
        if (this.state)
            this.state.subtitles = val;
    }
    static get playing() {
        return typeof this.element === 'undefined' ? false : !this.element.paused;
    }
    static get audioTimeout() {
        return this.state?.audioTimeout ?? 500;
    }
    static set audioTimeout(val) {
        if (this.state)
            this.state.audioTimeout = val;
    }
    static get playback() {
        return this.state?.playback ?? new Map();
    }
    static set playback(val) {
        if (this.state)
            this.state.playback = val;
    }
    static get playingState() {
        return this.state?.playingState ?? AudioPlayingState.Initial;
    }
    static set playingState(val) {
        if (this.state)
            this.state.playingState = val;
    }
    static get timerType() {
        return this.state?.timerType ?? 1;
    }
    static set timerType(val) {
        if (this.state)
            this.state.timerType = val;
    }
    static register(availableAudio) {
        ...
        this.playbackRatePlus?.addEventListener('click', (event) => {
            event.preventDefault();
            this.setPlaybackRate(this.playbackRate + 0.1);
        });
    ...
    static add(page, provider, language, voiceId) {
        const key = typeof language !== 'undefined' && typeof voiceId !== 'undefined'
            ...
            : provider;
        if (typeof this.availableAudio === 'undefined') {
            this.availableAudio = {};
        }
        const available = this.availableAudio;
        if (typeof available[key] === 'undefined') {
            available[key] = [];
        }
        if (!available[key].includes(page)) {
            available[key].push(page);
        }
        else {
    ...
    static async start(time, side) {
        if (typeof this.element === 'undefined' ||
            typeof this.provider === 'undefined' ||
            typeof this.availableAudio === 'undefined')
            return;
        if (this.playing)
            this.stopCurrentAudio();
        this.playingState = AudioPlayingState.Playing;
        ...
        if (Navigation.currentPage !== position.page)
            Navigation.currentPage = position.page;
        const audioPages = this.availableAudio[this.provider];
        if (typeof audioPages === 'undefined' ||
            !audioPages.includes(Navigation.currentPage)) {
            this.startTimerAfterAudio();
            return;
        }
