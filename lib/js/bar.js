...
export class Bar {
    ...
    static get state() {
        return Readiant.getInstance(Readiant.root)?.barState;
    }
    static get fontSize() {
        return this.state?.fontSize ?? 1;
    }
    static set fontSize(val) {
        if (this.state)
            this.state.fontSize = val;
    }
    static get readStop() {
        return this.state?.readStop ?? 1;
    }
    static set readStop(val) {
        if (this.state)
            this.state.readStop = val;
    }
    static get showing() {
        return this.state?.showing ?? false;
    }
    static set showing(val) {
        if (this.state)
            this.state.showing = val;
    }
    static register() {
