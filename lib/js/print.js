...
export class Print {
    ...
    static get originals() {
        return Readiant.getInstance(Readiant.root)?.printState.originals ?? [];
    }
    static set originals(val) {
        const state = Readiant.getInstance(Readiant.root)?.printState;
        if (state)
            state.originals = val;
    }
    static register() {
