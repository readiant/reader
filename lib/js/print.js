...
export class Print {
    static register() {
        ...
        const printChange = window.matchMedia('print');
    ...
    static after() {
        if (this.originals.length === 0)
            return;
        const pages = document.querySelectorAll('.rdnt__page:not(.hidden)');
...
Print.button = document.getElementsByClassName('rdnt__print-button')[0];
Print.viewport = document.getElementsByClassName('rdnt__viewport')[0];
Print.originals = [];
